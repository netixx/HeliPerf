...
sub assist {
  ...
  #troisième page-> étition de la base de donnée
  my $oEditBddTemplate = administration::ajouterWin::AjouterAssistPage::adaptation_bdd_template($combo->get_active_iter());
  $asswin->append_page($oEditBddTemplate);
  $asswin->set_page_title($oEditBddTemplate,"Édition de la base de donnée");
  $asswin->set_page_type($oEditBddTemplate,'content');#page de confirmation
  $asswin->set_page_complete($oEditBddTemplate, 1);

  #quatrième page -> choix des equipement présents en pesée
  my $oPresentPesee = administration::ajouterWin::AjouterAssistPage::choix_present_pesee();
  $asswin->append_page($oPresentPesee);
  $asswin->set_page_title($oPresentPesee,"Choix des équipement présents en pesée");
  $asswin->set_page_type($oPresentPesee,'content');#resumé des actions effectuées
  $asswin->set_page_complete($oPresentPesee, 1);

  #5ieme page -> choix de la configuration de base
  my $oConfigBase = administration::ajouterWin::AjouterAssistPage::choix_config_base();
  $asswin->append_page($oConfigBase);
  $asswin->set_page_title($oConfigBase,"Choix de la configuration de base");
  $asswin->set_page_type($oConfigBase,'content');#resumé des actions effectuées
  $asswin->set_page_complete($oConfigBase, 1);
  $asswin->show_all();
  ...
  $asswin->set_page_type($confirm_page,'confirm');#resumé des actions effectuées
